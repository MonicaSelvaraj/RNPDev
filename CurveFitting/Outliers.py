...
newC1s = numpy.delete (C1s, outliersIndex)
newC2s = numpy.delete (C2s, outliersIndex)
newC3s = numpy.delete (C3s, outliersIndex)

#Plots after removing outliers
plt.scatter(newC2s, newC3s, c='g'); plt.title(' dazl C2 vs C3'); plt.show() # Shows plot without first PC
plt.scatter(newC2s, newC1s, c='g'); plt.title(' dazl C1 vs C2'); plt.show() # Shows plot without third PC
plt.scatter(newC1s, newC3s, c='g'); plt.title(' dazl C1 vs C3'); plt.show()# Shows plot without second PC
...
popt, pcov = curve_fit(helixFit, newC1s, newC2s, p0=[5, 0.2, 0]) # Predicts C2 given C1
print("Fit parameters (radius, frequency, phase) for C1 -> C2:", popt)
C2Ps = [helixFit(c1, *popt) for c1 in newC1s]
#Output - [ 2.41007063  0.28832207 -0.23765725]

popt, pcov = curve_fit(helixFit, newC1s, newC3s, p0=[5, 0.2, -numpy.pi/2]) # Predicts C3 given C1
print("Fit parameters (radius, frequency, phase) for C1 -> C3:", popt)
C3Ps = [helixFit(c1, *popt) for c1 in newC1s]
#Output - [1.0358227  0.27418132 0.97085416]

#2D plots after fit
plt.scatter(newC2s, newC3s, c='g') # True
plt.plot(C2Ps, C3Ps, c='b') # Fit
plt.title(' dazl C2 vs C3 fit') 
plt.show()
plt.scatter(newC1s, newC2s, c='g') # True
plt.plot(newC1s, C2Ps, c= 'b') # Fit
plt.title(' dazl C1 vs C2 fit') 
plt.show()
plt.scatter(newC1s, newC3s, c='g') # True
plt.plot(newC1s, C3Ps, c='b') # Fit
...
for i in range(len(newC1s)-1): #Go from the center into the PC's direction by this much for each point
        #Plot3D, takes a start x, end x, start y, end y, start z, end z
        start = center + newC1s[i]*Pc1 + C2Ps[i]*Pc2 + C3Ps[i]*Pc3
        end = center + newC1s[i+1]*Pc1 + C2Ps[i+1]*Pc2 + C3Ps[i+1]*Pc3
