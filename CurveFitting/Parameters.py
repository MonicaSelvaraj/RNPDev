...
outliersIndex = numpy.where(dst>= 8)
print (outliersIndex)
#Removing the outliers from C1s, C2s, C3s
newC1sg = numpy.delete (C1sg, outliersIndex)
newC2sg = numpy.delete (C2sg, outliersIndex)
newC3sg = numpy.delete (C3sg, outliersIndex)


#Shows plots before fitting curves
plt.scatter(newC2sg, newC3sg, c='g'); plt.scatter(C2sr, C3sr, c='r'); plt.title(' dazl vasa C2 vs C3'); plt.show() # Shows plot without first PC
plt.scatter(newC2sg, newC1sg, c='g'); plt.scatter(C2sr, C1sr, c='r'); plt.title(' dazl vasa C1 vs C2'); plt.show() # Shows plot without third PC
plt.scatter(newC1sg, newC3sg, c='g'); plt.scatter(C1sr, C3sr, c='r'); plt.title(' dazl vasa C1 vs C3'); plt.show()# Shows plot without second PC
...
popt, pcov = curve_fit(helixFit, newC1sg, newC2sg, p0=[5, 0.4, -numpy.pi/2]) # Predicts C2 given C1
print("Fit parameters (radius, frequency, phase) for C1 -> C2:", popt)
C2Psg = [helixFit(c1, *popt) for c1 in newC1sg]
#Output - [ 2.41007063  0.28832207 -0.23765725]

popt, pcov = curve_fit(helixFit, newC1sg, newC3sg, p0=[5, 0.4, 0]) # Predicts C3 given C1
print("Fit parameters (radius, frequency, phase) for C1 -> C3:", popt)
C3Psg = [helixFit(c1, *popt) for c1 in newC1sg]
...
for i in range(len(newC1sg)-1): #Go from the center into the PC's direction by this much for each point
        #Plot3D, takes a start x, end x, start y, end y, start z, end z
        start = centerg + newC1sg[i]*Pc1g + C2Psg[i]*Pc2g + C3Psg[i]*Pc3g
        end = centerg + newC1sg[i+1]*Pc1g + C2Psg[i+1]*Pc2g + C3Psg[i+1]*Pc3g
...
for i in range(len(C1sr)-1): #Go from the center into the PC's direction by this much for each point
        ...
        #print (centerr+C1sr[i]*Pc1r); print (centerr+C2sr[i]*Pc2r); print (centerr+C3sr[i]*Pc3r)
