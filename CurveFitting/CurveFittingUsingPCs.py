...
center = numpy.array([27.61704192, 25.67052383,  8.83293281])#Vasa - C1
Pc1 = numpy.array([ 0.42995246,  0.74540579, -0.50942231])
Pc2 = numpy.array([ 0.90214046, -0.33230552,  0.27516473])
Pc3 = numpy.array([ 0.03582554, -0.57787823, -0.8153363 ])
...
plt.scatter(C1s, C2s) # Shows plot without first PC
plt.show()
...
popt, pcov = curve_fit(helixFit, C1s, C2s, p0=[10, 0.5, -numpy.pi/2]) # Predicts C2 given C1
print("Fit parameters (radius, frequency, phase) for C1 -> C2:", popt)
C2Ps = [helixFit(c1, *popt) for c1 in C1s]

popt, pcov = curve_fit(helixFit, C1s, C3s, p0=[10, 0.5, 0]) # Predicts C3 given C1
