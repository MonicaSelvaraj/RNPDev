...
xline = 5*numpy.sin((1/6)*zline) + 28.74435878
yline = 5*numpy.cos((1/6)*zline) + 25.34122088
...
[  9.24494445   9.46010829 422.53519124] - center of shifted helix
New shift : 
...
newAxisPts1= numpy.matmul(originalAxisPts, Ry)
newAxisPts = numpy.matmul(newAxisPts1, Rx)

newdatamean = newAxisPts.mean(axis = 0)
print (newdatamean)

newShift = datamean - newdatamean
print (newShift)

#print (newAxisPts)

newAxis = numpy.hsplit(newAxisPts, 3)
newxline = numpy.concatenate(newAxis[0]+newShift[0], axis=0)
newyline = numpy.concatenate(newAxis[1]+newShift[1], axis=0)
...
#Connecting points that are close
for x1, y1, z1 in zip(x2, y2, z2):
    for x3, y3, z3 in zip(X2, Y2, Z2):
        if x1 > x3: # This prevents repeats
            continue
        if (x1-x3)**2 + (y1-y3)**2 + (z1-z3)**2 < 100:
            ax.plot3D([x1, x3], [y1, y3], [z1, z3], color = 'red')
